  ...
  return (
    <div style={{
      ...
      {isDev && (
        <form onSubmit={handleDevLogin} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <button
            ...
            style={{
              padding: '12px', fontSize: 14, fontWeight: 500, borderRadius: 6,
              background: loading || !email.trim() ? 'var(--divider)' : 'var(--text-primary)',
              color: loading || !email.trim() ? 'var(--text-muted)' : 'var(--text-on-dark)',
      ...
      <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 24, textAlign: 'center' }}>
        Primeira vez?{' '}
        <Link href="/" style={{ color: 'var(--accent)' }}>Continue comprando</Link>
