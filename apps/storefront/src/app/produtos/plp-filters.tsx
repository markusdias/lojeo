...
export function PLPFilters({ products, currency, initialSort = 'novidades', initialQ }: PLPFiltersProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return out;
  }, [products, materials, priceMax, size, sort, q]);
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 56 }}>
      ...
      <aside>
        ...
        <details open style={{ marginBottom: 28 }}>
          <summary style={{ fontSize: 13, fontWeight: 500, cursor: 'pointer', marginBottom: 12, listStyle: 'none', display: 'flex', justifyContent: 'space-between' }}>
            Material <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>▾</span>
          </summary>
          {MATERIALS.map(m => (
            <label key={m.slug} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, cursor: 'pointer', fontSize: 14 }}>
              <input
                type="checkbox"
              ...
              />
              <span style={{ width: 14, height: 14, borderRadius: 999, background: m.swatch, border: '1px solid #00000020', flexShrink: 0 }} />
              {m.label}
            </label>
          ))}
        </details>
        ...
        <details open style={{ marginBottom: 28 }}>
          <summary style={{ fontSize: 13, fontWeight: 500, cursor: 'pointer', marginBottom: 12, listStyle: 'none', display: 'flex', justifyContent: 'space-between' }}>
            Preço até <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>▾</span>
          </summary>
          <input
            type="range"
            min={0}
        ...
        <details open style={{ marginBottom: 28 }}>
          <summary style={{ fontSize: 13, fontWeight: 500, cursor: 'pointer', marginBottom: 12, listStyle: 'none', display: 'flex', justifyContent: 'space-between' }}>
            Aro <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>▾</span>
          </summary>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {SIZES.map(s => (
              <button
                ...
                style={{
                  padding: '6px 10px', fontSize: 13, cursor: 'pointer', borderRadius: 2,
        ...
        {/* Limpar */}
        {(materials.length > 0 || size !== null || q) && (
          <button
            onClick={() => { setMaterials([]); setSize(null); setQ(''); setPriceMax(maxPrice); setPage(1); }}
            style={{
      ...
      <div>
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: '80px 0', textAlign: 'center' }}>
            ...
            <button
              onClick={() => { setMaterials([]); setSize(null); setQ(''); setPriceMax(maxPrice); }}
